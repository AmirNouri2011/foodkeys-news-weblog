...
const SITE_NAME = process.env.NEXT_PUBLIC_SITE_NAME || "وب‌لاگ فودکی";
const SITE_DESCRIPTION =
	process.env.NEXT_PUBLIC_SITE_DESCRIPTION ||
	"A modern news and articles platform";
...
const POST_DETAILS_PATH = "/view/news/details";

/**
 * Get canonical URL for a post (details page by id)
 */
export function getPostUrl(post: { id: number }): string {
	return `${POST_DETAILS_PATH}?id=${post.id}`;
}
...
export function generatePostMetadata(
	...
	const canonical =
		post.canonicalUrl || absoluteUrl(postPagePath || getPostUrl(post));
...
export function generateCategoryMetadata(
	...
	const title =
		category.metaTitle || `${category.name} - اخبار و مقالات و اخبار`;
	const description =
...
export function generateArticleJsonLd(
	...
	const postUrl = absoluteUrl(postPagePath || getPostUrl(post));
