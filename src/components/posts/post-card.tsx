    ...
    return (
      <Link href={`/posts/${post.slug}`}>
        <motion.div
          whileHover={{ x: 3 }}
          transition={{ type: 'spring', stiffness: 300, damping: 20 }}
          className={cn('group cursor-pointer py-4 border-b last:border-0', className)}
        >
  ...
  return (
    <Link href={`/posts/${post.slug}`}>
      <motion.div
        ...
        transition={{ type: 'spring', stiffness: 300, damping: 20 }}
      >
        <Card className={cn('overflow-hidden group cursor-pointer h-full flex flex-col', className)}>
...
export function PostCardSkeleton({ variant = 'default' }: { variant?: 'default' | 'featured' | 'compact' | 'horizontal' }) {
  if (variant === 'compact') {
    return (
